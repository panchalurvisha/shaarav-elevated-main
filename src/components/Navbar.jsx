...
export const Navbar = () => {
  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "bg-paper/90 backdrop-blur border-b border-border" : "bg-transparent"
        }`}
    >
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {links.map((l) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `text-sm tracking-wide transition-colors ${isActive ? "text-primary font-medium" : "text-cocoa/80 hover:text-primary"
                }`
              }
            >
              {l.label}
